...
const DeleteCustomer = () => {
  ...
  const handleSearch = async (e) => {
    ...
    setError('');
    try {
       response = await axios.get(`https://user-bank-info-management.vercel.app/api/customers/search?query=${searchTerm}`);
      if (response.data) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await axios.delete(`https://user-bank-info-management.vercel.app/api/deleteuser/:${state.currentUserId}`);
